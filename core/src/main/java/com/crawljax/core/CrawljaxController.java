...
public class CrawljaxController implements CrawlQueueManager {
	...
	public final void run() throws CrawljaxException {
		...
		while (!configurationReader.getCrawlSpecificationReader().endOfURLArray()){
			
			System.out.println("Size of URL is greater than 1, we are going to crawl the next url.");
			
			// Create the initial Crawler
			...
			configurationReader.updateLastUrlIndex();
			if (!configurationReader.getCrawlSpecificationReader().endOfURLArray()){
				workQueue = init();
		...
		System.out.println("Done calc time");
		/**
