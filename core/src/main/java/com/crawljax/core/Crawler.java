...
public class Crawler implements Runnable {
	...
	/**
	 * New version of goToInitialURL that adjusts the implementation of alsoCrawl()
	 * Same as the old one, it just use getSiteUrlNew instead of getSiteUrl
	 * 
	 */
	public void goToInitialURLNew(){
		// calling getSiteUrlNew() will move to next url. be careful!
		try {
			URL temp = new URL(configurationReader.getCrawlSpecificationReader().getSiteUrlNew().toString());
			System.out.println("Loading Page " + temp.toString());
			getBrowser().goToUrl(temp);
		} catch (MalformedURLException e) {
			e.printStackTrace();
			System.out.println("Cannot call getSiteUrlNew in goToInitialURLNew");
		}
