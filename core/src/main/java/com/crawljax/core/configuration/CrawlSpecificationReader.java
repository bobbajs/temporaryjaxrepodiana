...
public class CrawlSpecificationReader implements IgnoreFrameChecker {
	...
	/**
	 * @return the total number of URLs (specified by user) we need to crawl 
	 */
	public int getUrlArraySize(){
		return crawlSpecification.getUrlArraySize();
	}
	
	/**
	 * @return the URL of the site
	 * this function is the new version of getSiteURL
	 */
	public URL getSiteUrlNew(){
		return crawlSpecification.getNextURL();
	}

	/**
	 * Increment the current URL index in URL array
	 */
	public void updateUrlArrayIndex(){
		crawlSpecification.updateUrlArrayIndex();
	}
	
	/**
	 * @return true if we reach there are more URL to crawl, false otherwise
	 */
	public boolean endOfURLArray(){
		if (crawlSpecification.getLastURLCrawledIndex() == crawlSpecification.getUrlArraySize()){
			return true;
		} 
		return false;
	}
